  ...
  <section class="garmetix-page-stack">
    <div class="garmetix-dashboard-hero">
      <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p class="garmetix-kicker"><UIcon name="i-lucide-shield-check" class="size-4" /> Purchase return closeout</p>
          <h2 class="garmetix-dashboard-title">Return Settlement QA</h2>
          <p class="garmetix-dashboard-subtitle">
            Cross-checks purchase return, ITC reversal, debit-note and settlement evidence for the selected close date.
          </p>
        </div>
        <div class="flex flex-wrap gap-2">
          <UBadge :color="statusColor" variant="subtle">{{ report ? readText(report, ['status']) : 'Not run' }}</UBadge>
          <UButton icon="i-lucide-refresh-cw" color="primary" :loading="loading" @click="runReport">Run QA</UButton>
    ...
    <section class="grid gap-3 md:grid-cols-3">
      ...
      <div class="garmetix-metric-card">
        <p class="garmetix-metric-label">Return Amount</p>
        <p class="garmetix-metric-value">{{ money(readNumber(report, ['totalReturnAmount'])) }}</p>
        <p class="garmetix-metric-caption">ITC reversal {{ money(readNumber(report, ['totalItcReversalAmount'])) }}</p>
      </div>
      <div class="garmetix-metric-card">
        <p class="garmetix-metric-label">Settled / Refunded</p>
        <p class="garmetix-metric-value">{{ money(readNumber(report, ['totalSettledAmount'])) }}</p>
        <p class="garmetix-metric-caption">Refunds {{ money(readNumber(report, ['totalRefundAmount'])) }} - Open DN {{ money(readNumber(report, ['totalOpenDebitNoteAmount'])) }}</p>
      </div>
    </section>
      ...
      <ul v-if="issues.length" class="space-y-2">
        <li v-for="(issue, index) in issues" :key="index" class="border-b border-default pb-2 text-sm">
          <p class="font-medium">
            ...
            {{ readText(issue, ['returnNumber']) }} - {{ readText(issue, ['vendorName']) }} - {{ readText(issue, ['code']) }}
          </p>
    ...
    <section class="garmetix-section-card">
      <div class="mb-3 flex items-center justify-between">
        <h3 class="garmetix-panel-title">Return Evidence</h3>
        <p class="garmetix-panel-subtitle">{{ pagedEvidence.length }} of {{ evidenceRows.length }} return(s)</p>
      </div>
        ...
        <div class="overflow-x-auto">
          <table class="w-full min-w-[1080px] text-left text-sm">
            <thead class="bg-muted/30 text-xs uppercase text-muted">
              <tr>
                <th class="px-3 py-2 font-medium">Return No.</th>
                <th class="px-3 py-2 font-medium">Vendor</th>
                <th class="px-3 py-2 font-medium">Original Invoice</th>
                <th class="px-3 py-2 text-right font-medium">Header Tax</th>
                <th class="px-3 py-2 text-right font-medium">Item Tax</th>
                <th class="px-3 py-2 text-right font-medium">ITC Taxable</th>
                <th class="px-3 py-2 font-medium">Status</th>
              </tr>
            </thead>
            <tbody class="divide-y divide-default">
              <tr v-if="!pagedEvidence.length">
                <td colspan="7" class="px-3 py-8 text-center text-muted">No return evidence rows found.</td>
              </tr>
              <tr v-for="row in pagedEvidence" :key="readText(row, ['purchaseReturnId'])">
                <td class="max-w-40 truncate px-3 py-2 font-medium">{{ readText(row, ['returnNumber']) }}</td>
                <td class="max-w-44 truncate px-3 py-2">{{ readText(row, ['vendorName']) }}</td>
                <td class="max-w-40 truncate px-3 py-2">{{ readText(row, ['originalInvoiceNumber']) }}</td>
                <td class="px-3 py-2 text-right" :class="Math.abs(readNumber(row, ['headerTaxAmount']) - readNumber(row, ['itemTaxAmount'])) > 1 ? 'text-error' : ''">{{ money(readNumber(row, ['headerTaxAmount'])) }}</td>
                <td class="px-3 py-2 text-right">{{ money(readNumber(row, ['itemTaxAmount'])) }}</td>
                <td class="px-3 py-2 text-right">{{ money(readNumber(row, ['itcTaxableAmount'])) }}</td>
...
useHead({ title: 'Return Settlement QA - Garmetix Back Office' })
...
async function runReport() {
  ...
  error.value = ''
  try {
    report.value = await get<ApiRecord>('purchase-return/advanced-settlement', { from: filters.from, to: filters.to })
  } catch (caught) {
    error.value = caught instanceof Error ? caught.message : 'Unable to run purchase return settlement QA.'
  } finally {
    loading.value = false
  }
}
...
async function downloadEvidence() {
  ...
  error.value = ''
  try {
    await download('purchase-return/advanced-settlement/evidence.csv', { from: filters.from, to: filters.to }, `purchase-return-advanced-settlement-${filters.from}-${filters.to}.csv`)
  } catch (caught) {
    error.value = caught instanceof Error ? caught.message : 'Unable to download settlement QA evidence CSV.'
  } finally {
