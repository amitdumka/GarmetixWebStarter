  ...
  <section class="garmetix-page-stack">
    <div class="garmetix-dashboard-hero">
      <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p class="garmetix-kicker"><UIcon name="i-lucide-file-minus-2" class="size-4" /> Vendor / supplier debits</p>
          <h2 class="garmetix-dashboard-title">Debit Notes</h2>
          <p class="garmetix-dashboard-subtitle">
            Review vendor, supplier and party debits with their source documents and settlement status.
          </p>
        </div>
        <div class="flex flex-wrap gap-2">
          <UButton icon="i-lucide-plus" color="primary" variant="solid" to="/debit-notes/new">New Debit Note</UButton>
    ...
    <section class="garmetix-section-card">
      <div class="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h3 class="garmetix-panel-title">Debit Note Register</h3>
          <p class="garmetix-panel-subtitle">{{ filteredRows.length }} of {{ notes.length }} debit notes</p>
        </div>
        <UInput v-model="search" icon="i-lucide-search" placeholder="Search debit notes" class="sm:w-72" />
      </div>

      <BooksMasterTable :columns="columns" :rows="filteredRows" empty-text="No debit notes yet.">
        <template #actions="{ row }">
          <div class="flex flex-wrap gap-1">
            <UButton icon="i-lucide-pencil" size="xs" color="neutral" variant="ghost" :to="`/debit-notes/${row.id}`">Edit</UButton>
...
useHead({ title: 'Debit Notes - Garmetix Books' })
...
async function refresh() {
  ...
  error.value = ''
  try {
    notes.value = toRows(await get<unknown>('commercial-notes', { take: 150, noteType: 0 }))
  } catch (caught) {
    error.value = caught instanceof Error ? caught.message : 'Unable to load debit notes.'
  } finally {
    loading.value = false
  }
}
...
async function downloadNote(row: ApiRecord, a5Slip: boolean) {
  ...
  error.value = ''
  try {
    await download(`commercial-notes/${id}/pdf`, { a5Slip, signatures: true }, `${readText(row, ['number'], 'debit-note')}.pdf`)
  } catch (caught) {
    error.value = caught instanceof Error ? caught.message : 'Unable to download debit note.'
  } finally {
    downloading.value = ''
  }
}
