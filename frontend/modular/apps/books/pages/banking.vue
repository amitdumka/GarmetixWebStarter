  ...
  <section class="garmetix-page-stack">
    <div class="garmetix-dashboard-hero">
      <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p class="garmetix-kicker"><UIcon name="i-lucide-landmark" class="size-4" /> Books master data</p>
          <h2 class="garmetix-dashboard-title">Banking</h2>
          <p class="garmetix-dashboard-subtitle">
            Bank accounts, bank transactions, reconciliation, cheques, vendor banks and net-banking account details. Moved out of <NuxtLink to="/accounting" class="underline">Accounting</NuxtLink> for a cleaner workspace.
          </p>
        </div>
      ...
      <div class="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex flex-col gap-2 sm:flex-row">
          <USelect
            ...
            class="sm:w-64"
          />
          <UInput v-model="search" icon="i-lucide-search" placeholder="Search banking data" class="sm:w-72" />
        </div>
      </div>

      <BooksMasterTable :columns="currentColumns" :rows="pagedRows" empty-text="No banking rows found.">
...
useHead({ title: 'Banking - Garmetix Books' })

type BankingTab = 'bankAccounts' | 'bankTransactions' | 'reconciliation' | 'cheques' | 'vendorBanks' | 'accountDetails'
...
const message = ref('')
const search = ref('')
const activeTab = ref<BankingTab>('bankAccounts')
...
const canCreate = computed(() => !['reconciliation'].includes(activeTab.value))
...
function singularLabel(tab: BankingTab) {
  return {
...
function findRowById(id: unknown): ApiRecord | null {
  const source: Record<BankingTab, ApiRecord[]> = {
...
const tabs: Array<{ key: BankingTab, label: string, icon: string, description: string }> = [
...
const tableRows = computed<Record<BankingTab, ApiRecord[]>>(() => ({
...
const columns: Record<BankingTab, Array<{ key: string, label: string }>> = {
  bankAccounts: [
...
watch([search, pageSize, activeTab], () => { page.value = 1 })
...
function resolveCompanyId() {
  ...
  if (!companyId) throw new Error('Run quick setup before saving banking masters.')
  return companyId
}
...
async function refresh() {
  ...
  try {
    const [
      ...
      transactionData, chequeData, vendorBankData, accountDetailData, vendorData
    ...
    const failedReasons = [ledgerData, bankData, bankAccountData, transactionData, chequeData, vendorBankData, accountDetailData]
      .filter((item): item is PromiseRejectedResult => item.status === 'rejected')
      .map(item => item.reason instanceof Error ? item.reason.message : String(item.reason))
    if (failedReasons.length) error.value = failedReasons[0]
  } catch (caught) {
    error.value = caught instanceof Error ? caught.message : 'Unable to load banking masters.'
  } finally {
    loading.value = false
  }
}
...
async function markStatementReconciled(item: ApiRecord | null) {
  ...
  try {
    await post<unknown>(`accounting/bank-statement-lines/${item.id}/reconcile`, {
      ...
      remarks: 'Marked reconciled from Books banking workspace'
    })
...
async function undoStatementReconciliation(item: ApiRecord | null) {
  ...
  error.value = ''
  try {
    await post<unknown>(`accounting/bank-statement-lines/${item.id}/unreconcile`, { remarks: 'Reconciliation reopened from Books banking workspace' })
...
async function updateChequeStatus(item: ApiRecord | null, status: string) {
  ...
  try {
    await post<unknown>(`accounting/cheque-logs/${item.id}/lifecycle`, {
      ...
      remarks: `Marked ${status.toLowerCase()} from Books banking workspace`,
      bankTransactionId: item.bankTransactionId || null
    })
