      ...
      <table class="w-full min-w-[720px] text-left text-sm">
        ...
        <tbody class="divide-y divide-default">
          <tr v-if="rows.length === 0">
            <td :colspan="(columns.length || 1) + ($slots.actions ? 1 : 0)" class="px-3 py-8 text-center text-muted">
              {{ emptyText }}
            </td>
          </tr>
          <tr v-for="(row, index) in rows" :key="rowKey(row, index)" class="bg-default/40">
            <td v-for="column in columns" :key="column.key" class="max-w-72 truncate px-3 py-2">
...
import { readText, type ApiRecord } from '../utils/books-api'

const { columns = [], rows = [], emptyText = 'No rows found.' } = defineProps<{
...
function rowKey(row: ApiRecord, index: number) {
  return readText(row, ['id', 'ledgerId', 'bankAccountId', 'name'], String(index))
}
