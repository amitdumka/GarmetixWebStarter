<template>
  <section class="garmetix-page-stack">
    <div class="garmetix-dashboard-hero">
      <div class="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
        ...
        <div class="flex flex-wrap gap-2">
          ...
          <UButton icon="i-lucide-plus" color="primary" @click="resetForm">New Policy</UButton>
        </div>
      </div>
    </div>
    ...
    <div class="grid gap-4 xl:grid-cols-[minmax(380px,0.8fr)_minmax(0,1.35fr)]">
      <form class="garmetix-section-card space-y-3" @submit.prevent="save">
        <div class="garmetix-panel-header">
          <div>
            <h3 class="garmetix-panel-title">{{ editingId ? 'Edit Policy' : 'New Policy' }}</h3>
      ...
      <div class="grid content-start gap-3 md:grid-cols-2">
        <div v-for="policy in policies" :key="readText(policy, ['id'])" class="garmetix-row-card">
          ...
          <div class="mt-3 flex justify-end gap-2">
            <UButton size="xs" icon="i-lucide-pencil" color="primary" variant="soft" @click="edit(policy)">Edit</UButton>
          </div>
        </div>
...
const { get, post, put } = useHrApiClient()
