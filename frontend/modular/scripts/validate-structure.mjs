import { existsSync, readFileSync } from 'node:fs'
import { join } from 'node:path'
import { fileURLToPath } from 'node:url'

const root = fileURLToPath(new URL('..', import.meta.url))
const requiredPaths = [
  '../../README.md',
  '../../backend/Garmetix.Api/Garmetix.Api.csproj',
  '../legacy/garmetix-web/package.json',
  'README.md',
  '.env.example',
  'package.json',
  'config/apps.ts',
  'config/routes.ts',
  'config/version.ts',
  'docs/stage-01-foundation.md',
  'docs/cloudflare-subdomains.md',
  'docs/MODULAR_TODO.md',
  'docs/stage-12a3-route-ownership.md',
  'docs/stage-12a4-shell-menu-links.md',
  'docs/stage-12a5-shared-shell-status.md',
  'docs/stage-12b1-pos-foundation.md',
  'docs/stage-12b2-pos-sale-draft.md',
  'docs/stage-12b3-pos-customer-print-queue.md',
  'docs/stage-12b4-pos-operator-hardening.md',
  'docs/stage-12b5-pos-returns-foundation.md',
  'docs/stage-12b6-pos-held-bills.md',
  'docs/stage-12b7-pos-day-open-close.md',
  'docs/stage-12b8-pos-static-deploy.md',
  'docs/stage-12c1-hr-foundation.md',
  'docs/stage-12c2-hr-safe-actions.md',
  'docs/stage-12c3-hr-static-deploy.md',
  'docs/stage-12d1-ai-sense-foundation.md',
  'docs/stage-12d2-ai-sense-analytics-endpoints.md',
  'docs/stage-12d3-ai-sense-static-deploy.md',
  'docs/stage-12e1-books-foundation.md',
  'docs/stage-12e2-books-master-data.md',
  'docs/stage-12e3-books-bank-operations.md',
  'docs/stage-12e4-books-voucher-review.md',
  'docs/stage-12e5-books-petty-cash-review.md',
  'docs/stage-12e6-books-vendor-payment-review.md',
  'docs/stage-12e7-books-gst-review.md',
  'docs/stage-12e8-books-audit-controls.md',
  'docs/stage-12e9-books-static-deploy.md',
  'docs/stage-12f1-admin-foundation.md',
  'docs/stage-12f2-admin-safe-diagnostics.md',
  'docs/stage-12f3-admin-static-deploy.md',
  'docs/stage-12g1-main-back-office-route-shell.md',
  'docs/stage-12g2-main-readonly-dashboard-data.md',
  'docs/stage-12g3-main-readonly-operations-review.md',
  'docs/stage-12g4-main-static-deploy.md',
  'docs/stage-12h1-deployment-split-guide.md',
  'docs/stage-12h2-all-app-validation.md',
  'docs/stage-12h3-deploy-preflight.md',
  'docs/stage-12h4-host-deployment-notes.md',
  'docs/stage-12h5-release-checklist.md',
  'docs/stage-12h6-deployment-acceptance.md',
  'docs/stage-12z-final-verification.md',
  'docs/stage-13a1-production-hardening-foundation.md',
  'docs/stage-13a2-local-route-smoke-tests.md',
  'docs/stage-13a3-api-auth-smoke-checks.md',
  'docs/stage-13a4-public-url-smoke-reporting.md',
  'docs/stage-13a5-visual-smoke-notes.md',
  'docs/stage-13b1-pos-sale-contract-parity.md',
  'docs/stage-13b2-pos-local-workflow-hardening.md',
  'docs/stage-13b3-pos-print-recovery-document-search.md',
  'docs/stage-13b4-pos-store-day-print-recovery.md',
  'docs/stage-13b5-pos-return-contract-parity.md',
  'docs/stage-13b6-pos-exchange-workflow.md',
  'docs/stage-13b7-pos-operator-acceptance.md',
  'docs/stage-13b8-pos-server-held-bills.md',
  'docs/stage-13b9-pos-held-bill-smoke.md',
  'docs/stage-13b10-pos-held-bill-browser-acceptance.md',
  'docs/stage-13b11-pos-save-after-resume-readiness.md',
  'docs/stage-13b12-pos-live-save-fixture-readiness.md',
  'docs/stage-13b-final-pos-closure.md',
  'docs/stage-13c1-hr-payroll-readiness-foundation.md',
  'docs/stage-13c2-hr-attendance-contracts.md',
  'docs/stage-13c3-hr-browser-acceptance.md',
  'docs/stage-13c4-hr-device-bridge-readiness.md',
  'docs/stage-13c5-hr-payroll-preview-readiness.md',
  'docs/stage-13c-final-hr-closure.md',
  'docs/stage-13d1-books-accounting-readiness.md',
  'docs/stage-13d2-books-accounting-contracts.md',
  'docs/stage-13d3-books-browser-acceptance.md',
  'docs/stage-13d4-books-ledger-sync-readiness.md',
  'docs/stage-13d5-books-posting-preflight.md',
  'docs/stage-13d-final-books-closure.md',
  'docs/stage-13e1-main-backoffice-readiness.md',
  'docs/stage-13e2-main-backoffice-contracts.md',
  'docs/stage-13e3-main-browser-acceptance.md',
  'docs/stage-13e4-main-writable-readiness.md',
  'docs/stage-13e-final-main-closure.md',
  'docs/stage-13f1-admin-saas-readiness.md',
  'docs/stage-13f2-admin-browser-acceptance.md',
  'docs/stage-13f3-admin-writable-preflight.md',
  'docs/stage-13f-final-admin-closure.md',
  'docs/stage-13g1-srp-deployment-foundation.md',
  'docs/stage-13g2-srp-host-readiness.md',
  'docs/stage-13g3-srp-deployment-packaging.md',
  'docs/stage-13g4-srp-runtime-bootstrap.md',
  'docs/stage-13g5-srp-cloudflare-activation.md',
  'docs/stage-13g6-srp-public-acceptance.md',
  'docs/stage-13g7-srp-same-origin-api-fix.md',
  'docs/stage-14a1-pos-parity-baseline.md',
  'docs/stage-14a2-pos-live-safe-deploy-gate.md',
  'docs/stage-14a3-pos-controlled-live-sale-acceptance.md',
  'docs/stage-14a4-pos-controlled-return-exchange-acceptance.md',
  'docs/stage-14a5-pos-operations-recovery-acceptance.md',
  'docs/stage-14a6-pos-cashier-workflow-acceptance.md',
  'docs/stage-14a7-pos-final-closure.md',
  'docs/stage-14a8-pos-sale-ui-search-layout-repair.md',
  'docs/stage-14b1-hr-parity-baseline.md',
  'docs/stage-14b2-hr-salary-payment-preview-repair.md',
  'docs/stage-14b3-hr-attendance-monthly-readiness.md',
  'docs/stage-14b4-hr-manual-punch-regularization.md',
  'docs/stage-14b5-hr-attendance-guarded-actions.md',
  'docs/stage-14b6-hr-attendance-device-kiosk-readiness.md',
  'docs/stage-14b7-hr-payroll-approval-evidence.md',
  'docs/stage-14b8-hr-live-payroll-acceptance.md',
  'docs/stage-14b9-hr-final-closure.md',
  'docs/stage-14b10-hr-legacy-full-parity-audit.md',
  'docs/stage-14b11-hr-listed-route-parity.md',
  'docs/stage-14c1-books-parity-baseline.md',
  'docs/stage-14c2-books-voucher-ledger-parity.md',
  'docs/stage-14c3-books-bank-reconciliation-parity.md',
  'docs/stage-14c4-books-bank-operations-parity.md',
  'docs/stage-14c5-books-gst-fy-lock-closure.md',
  'docs/stage-14g-books-gst-full-parity.md',
  'docs/stage-14d1-crm-digital-crm-foundation.md',
  'docs/stage-14d2-crm-customer-master-parity.md',
  'docs/stage-14d2a-crm-nuxt-ui-layout-hotfix.md',
  'docs/stage-14d3-crm-loyalty-dues-parity.md',
  'docs/stage-14d4-crm-runtime-digital-bill-port.md',
  'docs/stage-14d5-crm-customer-register-filters.md',
  'docs/stage-14d6-crm-feedback-whatsapp-operations.md',
  'docs/stage-14d7-crm-campaign-operations.md',
  'docs/stage-14d8-crm-ad-banners-public-bill.md',
  'docs/stage-14d9-crm-final-closure.md',
  'docs/stage-14f1-ai-assistant-mcp-intake.md',
  'docs/stage-14f2-assistant-backend-foundation.md',
  'docs/stage-14f3-assistant-frontend-panel.md',
  'docs/stage-14f4-ai-sense-tool-catalog.md',
  'docs/stage-14f5-ai-sense-api-path-hotfix.md',
  'docs/stage-14f6-ai-sense-runtime-path-guard.md',
  'docs/deployment-validation-cadence.md',
  'deploy/README.md',
  'deploy/main-static-deploy.sh',
  'deploy/pos-static-deploy.sh',
  'deploy/hr-static-deploy.sh',
  'deploy/ai-sense-static-deploy.sh',
  'deploy/books-static-deploy.sh',
  'deploy/crm-static-deploy.sh',
  'deploy/admin-static-deploy.sh',
  'deploy/srp-deploy.config.example.env',
  'deploy/srp-whole-site-deploy.sh',
  'deploy/srp-host-readiness.sh',
  'deploy/srp-runtime-bootstrap.sh',
  'deploy/srp-cloudflare-activate.sh',
  'scripts/validate-all.mjs',
  'scripts/run-bash-script.mjs',
  'scripts/deploy-preflight.mjs',
  'scripts/srp-public-acceptance.mjs',
  'scripts/release-checklist.mjs',
  'scripts/smoke-routes.mjs',
  'scripts/smoke-checklist.mjs',
  'scripts/smoke-local-routes.mjs',
  'scripts/smoke-api-auth.mjs',
  'scripts/smoke-public-report.mjs',
  'scripts/smoke-visual-notes.mjs',
  'scripts/pos-sale-contract-check.mjs',
  'scripts/pos-operator-acceptance.mjs',
  'scripts/pos-held-bill-smoke.mjs',
  'scripts/pos-held-bill-browser-acceptance.mjs',
  'scripts/pos-save-after-resume-readiness.mjs',
  'scripts/pos-live-save-fixture-readiness.mjs',
  'scripts/pos-stage13b-closure.mjs',
  'scripts/pos-parity-baseline.mjs',
  'scripts/pos-live-safe-deploy-gate.mjs',
  'scripts/pos-live-sale-acceptance.mjs',
  'scripts/pos-live-return-exchange-acceptance.mjs',
  'scripts/pos-operations-recovery-acceptance.mjs',
  'scripts/pos-cashier-workflow-acceptance.mjs',
  'scripts/pos-final-closure.mjs',
  'deploy/srp-backup-database.sh',
  'scripts/hr-payroll-readiness.mjs',
  'scripts/hr-parity-baseline.mjs',
  'scripts/hr-salary-payment-preview-contract.mjs',
  'scripts/hr-attendance-contract-check.mjs',
  'scripts/hr-attendance-monthly-readiness.mjs',
  'scripts/hr-manual-punch-regularization-readiness.mjs',
  'scripts/hr-attendance-guarded-actions-readiness.mjs',
  'scripts/hr-payroll-approval-evidence-readiness.mjs',
  'scripts/hr-live-payroll-acceptance.mjs',
  'scripts/hr-final-closure.mjs',
  'scripts/hr-legacy-parity-audit.mjs',
  'scripts/hr-listed-route-parity.mjs',
  'scripts/hr-payroll-device-final-parity.mjs',
  'scripts/hr-browser-acceptance.mjs',
  'scripts/hr-device-bridge-readiness.mjs',
  'scripts/hr-payroll-preview-readiness.mjs',
  'scripts/hr-stage13c-closure.mjs',
  'scripts/books-accounting-readiness.mjs',
  'scripts/books-accounting-contract-check.mjs',
  'scripts/books-browser-acceptance.mjs',
  'scripts/books-ledger-sync-readiness.mjs',
  'scripts/books-posting-preflight.mjs',
  'scripts/books-parity-baseline.mjs',
  'scripts/books-voucher-ledger-parity.mjs',
  'scripts/books-bank-reconciliation-parity.mjs',
  'scripts/books-bank-operations-parity.mjs',
  'scripts/books-stage13d-closure.mjs',
  'scripts/books-stage14c5-closure.mjs',
  'scripts/books-stage14g-closure.mjs',
  'scripts/crm-final-closure.mjs',
  'scripts/assistant-mcp-intake-readiness.mjs',
  'scripts/assistant-backend-foundation-readiness.mjs',
  'scripts/assistant-frontend-panel-readiness.mjs',
  'scripts/assistant-tool-catalog-readiness.mjs',
  'scripts/ai-sense-api-path-readiness.mjs',
  'scripts/workspace-link-readiness.mjs',
  'scripts/main-backoffice-readiness.mjs',
  'scripts/main-backoffice-contract-check.mjs',
  'scripts/main-backoffice-browser-acceptance.mjs',
  'scripts/main-writable-readiness.mjs',
  'scripts/main-stage13e-closure.mjs',
  'scripts/admin-saas-readiness.mjs',
  'scripts/admin-browser-acceptance.mjs',
  'scripts/admin-writable-preflight.mjs',
  'scripts/admin-stage13f-closure.mjs',
  'packages/shared-api/src/index.ts',
  'packages/shared-auth/src/index.ts',
  'packages/shared-types/src/index.ts',
  'packages/shared-utils/src/index.ts',
  'packages/shared-ui/src/index.ts',
  'packages/shared-ui/components/GarmetixAssistantPanel.vue',
  'apps/main/package.json',
  'apps/main/app.vue',
  'apps/main/utils/main-api.ts',
  'apps/main/components/MainPlaceholder.vue',
  'apps/main/components/MainDashboardReadModel.vue',
  'apps/main/components/MainReadOnlyTable.vue',
  'apps/main/middleware/auth.global.ts',
  'apps/main/pages/index.vue',
  'apps/main/pages/login.vue',
  'apps/main/pages/dashboard/index.vue',
  'apps/main/pages/dashboard/todays.vue',
  'apps/main/pages/dashboard/store-manager.vue',
  'apps/main/pages/dashboard/map.vue',
  'apps/main/pages/store-day.vue',
  'apps/main/pages/billing/index.vue',
  'apps/main/pages/tailoring.vue',
  'apps/main/pages/purchase/index.vue',
  'apps/main/pages/purchase/new.vue',
  'apps/main/pages/purchase-return/index.vue',
  'apps/main/pages/purchase-return/goods-return.vue',
  'apps/main/pages/inventory.vue',
  'apps/main/pages/stock-operations.vue',
  'apps/main/pages/customers/index.vue',
  'apps/main/pages/customers/new.vue',
  'apps/main/pages/customers/[id].vue',
  'apps/main/pages/reports.vue',
  'apps/main/pages/document-scan.vue',
  'apps/main/pages/profile.vue',
  'apps/main/pages/access-denied.vue',
  'apps/main/pages/about-us.vue',
  'apps/main/pages/contact-us.vue',
  'apps/main/pages/faq.vue',
  'apps/main/pages/i/[token].vue',
  'apps/main/pages/[module].vue',
  'apps/pos/package.json',
  'apps/pos/pages/index.vue',
  'apps/pos/pages/login.vue',
  'apps/pos/pages/day-open.vue',
  'apps/pos/pages/sale.vue',
  'apps/pos/pages/hold-bills.vue',
  'apps/pos/pages/returns.vue',
  'apps/pos/pages/exchange.vue',
  'apps/pos/pages/print.vue',
  'apps/pos/pages/day-close.vue',
  'apps/pos/utils/local-pos-storage.ts',
  'apps/pos/utils/pos-documents.ts',
  'apps/pos/utils/return-contract.ts',
  'apps/pos/utils/sale-contract.ts',
  'apps/hr/package.json',
  'apps/hr/pages/index.vue',
  'apps/hr/pages/login.vue',
  'apps/hr/pages/employees.vue',
  'apps/hr/pages/attendance/today.vue',
  'apps/hr/pages/attendance/monthly.vue',
  'apps/hr/pages/attendance/manual-punch.vue',
  'apps/hr/pages/attendance/payroll-summary.vue',
  'apps/hr/pages/attendance/payroll-review.vue',
  'apps/hr/pages/attendance/salary-draft.vue',
  'apps/hr/pages/attendance/regularization.vue',
  'apps/hr/pages/payroll.vue',
  'apps/hr/pages/payroll/finalization.vue',
  'apps/hr/pages/attendance/salary-payment.vue',
  'apps/hr/pages/attendance/shift-rules.vue',
  'apps/hr/pages/attendance/devices.vue',
  'apps/hr/middleware/auth.global.ts',
  'apps/ai-sense/package.json',
  'apps/ai-sense/pages/index.vue',
  'apps/ai-sense/pages/login.vue',
  'apps/ai-sense/pages/dashboard/business.vue',
  'apps/ai-sense/pages/stock-reports.vue',
  'apps/ai-sense/pages/sales-analysis.vue',
  'apps/ai-sense/pages/purchase-analysis.vue',
  'apps/ai-sense/pages/profit-analysis.vue',
  'apps/ai-sense/pages/stock-risk.vue',
  'apps/ai-sense/pages/vendor-analysis.vue',
  'apps/ai-sense/pages/customer-analysis.vue',
  'apps/ai-sense/pages/daily-summary.vue',
  'apps/ai-sense/pages/monthly-summary.vue',
  'apps/ai-sense/components/AiConnectedAnalysis.vue',
  'apps/ai-sense/middleware/auth.global.ts',
  'apps/books/package.json',
  'apps/books/pages/index.vue',
  'apps/books/pages/login.vue',
  'apps/books/pages/accounting.vue',
  'apps/books/pages/banking.vue',
  'apps/books/pages/ledgers.vue',
  'apps/books/pages/financial-year-locks.vue',
  'apps/books/pages/petty-cash.vue',
  'apps/books/pages/cash-details.vue',
  'apps/books/pages/vouchers.vue',
  'apps/books/pages/vendor-payments.vue',
  'apps/books/pages/vendor-settlements.vue',
  'apps/books/pages/commercial-notes.vue',
  'apps/books/pages/parties.vue',
  'apps/books/pages/gst-returns.vue',
  'apps/books/pages/gst-reports.vue',
  'apps/books/pages/gst-production.vue',
  'apps/books/pages/accounting-gst-validation.vue',
  'apps/books/composables/useGstReviewContact.ts',
  'apps/books/pages/audit.vue',
  'apps/books/pages/message-logs.vue',
  'apps/books/pages/debit-notes/index.vue',
  'apps/books/pages/debit-notes/new.vue',
  'apps/books/pages/debit-notes/[id].vue',
  'apps/books/pages/credit-notes/index.vue',
  'apps/books/pages/credit-notes/new.vue',
  'apps/books/pages/credit-notes/[id].vue',
  'apps/books/components/BooksPlaceholder.vue',
  'apps/books/components/BooksMasterTable.vue',
  'apps/books/middleware/auth.global.ts',
  'apps/books/utils/books-api.ts',
  'apps/crm/package.json',
  'apps/crm/app.vue',
  'apps/crm/nuxt.config.ts',
  'apps/crm/pages/index.vue',
  'apps/crm/pages/login.vue',
  'apps/crm/pages/customers/index.vue',
  'apps/crm/pages/customers/new.vue',
  'apps/crm/pages/customers/[id].vue',
  'apps/crm/pages/customers/dues-reconciliation.vue',
  'apps/crm/pages/loyalty.vue',
  'apps/crm/pages/marketing/digital-bills.vue',
  'apps/crm/pages/marketing/digital-bill-analytics.vue',
  'apps/crm/pages/marketing/campaign-audiences.vue',
  'apps/crm/pages/marketing/campaigns.vue',
  'apps/crm/pages/marketing/customer-feedback.vue',
  'apps/crm/pages/marketing/review-settings.vue',
  'apps/crm/pages/marketing/whatsapp-settings.vue',
  'apps/crm/pages/marketing/whatsapp-logs.vue',
  'apps/crm/pages/marketing/ad-banners.vue',
  'apps/crm/pages/marketing/digital-bill-acceptance.vue',
  'apps/crm/components/CrmMasterTable.vue',
  'apps/crm/middleware/auth.global.ts',
  'apps/crm/utils/crm-api.ts',
  'apps/admin/package.json'
  ,
  'apps/admin/app.vue',
  'apps/admin/pages/index.vue',
  'apps/admin/pages/login.vue',
  'apps/admin/pages/access-denied.vue',
  'apps/admin/pages/setup.vue',
  'apps/admin/pages/access.vue',
  'apps/admin/pages/message-logs.vue',
  'apps/admin/pages/system-health.vue',
  'apps/admin/pages/runtime-diagnostics.vue',
  'apps/admin/pages/production-readiness.vue',
  'apps/admin/pages/license-activation.vue',
  'apps/admin/pages/import-export.vue',
  'apps/admin/pages/data-consistency.vue',
  'apps/admin/pages/client-onboarding.vue',
  'apps/admin/pages/backup-maintenance.vue',
  'apps/admin/pages/google-drive-backup.vue',
  'apps/admin/pages/production-support.vue',
  'apps/admin/pages/production-rehearsal.vue',
  'apps/admin/pages/gst-final-acceptance.vue',
  'apps/admin/components/AdminMasterTable.vue',
  'apps/admin/components/AdminPlaceholder.vue',
  'apps/admin/components/SupportDrillPage.vue',
  'apps/admin/middleware/auth.global.ts',
  'apps/admin/utils/admin-api.ts',
  'apps/final-accounts/package.json',
  'apps/final-accounts/app.vue',
  'apps/final-accounts/nuxt.config.ts',
  'apps/final-accounts/pages/index.vue',
  'apps/final-accounts/pages/setup.vue',
  'apps/final-accounts/pages/chart-of-accounts.vue',
  'apps/final-accounts/pages/fiscal-periods.vue',
  'apps/final-accounts/pages/login.vue',
  'apps/final-accounts/pages/access-denied.vue',
  'apps/final-accounts/middleware/auth.global.ts',
  'apps/final-accounts/utils/final-accounts-api.ts'
]

const failures = []
for (const relativePath of requiredPaths) {
  const fullPath = relativePath.startsWith('../')
    ? join(root, relativePath)
    : join(root, relativePath)
  if (!existsSync(fullPath)) {
    failures.push(`Missing ${relativePath}`)
  }
}

const env = readFileSync(join(root, '.env.example'), 'utf8')
for (const key of [
  'NUXT_PUBLIC_GARMETIX_API_BASE_URL',
  'NUXT_PUBLIC_GARMETIX_MAIN_URL',
  'NUXT_PUBLIC_GARMETIX_POS_URL',
  'NUXT_PUBLIC_GARMETIX_HR_URL',
  'NUXT_PUBLIC_GARMETIX_AI_SENSE_URL',
  'NUXT_PUBLIC_GARMETIX_BOOKS_URL',
  'NUXT_PUBLIC_GARMETIX_CRM_URL',
  'NUXT_PUBLIC_GARMETIX_ADMIN_URL',
  'NUXT_PUBLIC_GARMETIX_FINAL_ACCOUNTS_URL',
  'NUXT_PUBLIC_GARMETIX_MAIN_BASE_PATH',
  'NUXT_PUBLIC_GARMETIX_POS_BASE_PATH',
  'NUXT_PUBLIC_GARMETIX_HR_BASE_PATH',
  'NUXT_PUBLIC_GARMETIX_AI_SENSE_BASE_PATH',
  'NUXT_PUBLIC_GARMETIX_BOOKS_BASE_PATH',
  'NUXT_PUBLIC_GARMETIX_CRM_BASE_PATH',
  'NUXT_PUBLIC_GARMETIX_ADMIN_BASE_PATH',
  'NUXT_PUBLIC_GARMETIX_FINAL_ACCOUNTS_BASE_PATH'
]) {
  if (!env.includes(key)) failures.push(`Missing env key ${key}`)
}

const packageJson = JSON.parse(readFileSync(join(root, 'package.json'), 'utf8'))
if (packageJson.devDependencies?.['@nuxt/ui'] !== '4.9.0') {
  failures.push('Nuxt UI 4.9.0 dependency is not pinned exactly in frontend/modular/package.json')
}

if (failures.length > 0) {
  console.error(failures.join('\n'))
  process.exit(1)
}

console.log('Garmetix modular folder structure validation passed.')
