...
`frontend/modular/deploy/srp-cloudflare-activate.sh` now:
...
```bash
bash frontend/modular/deploy/srp-cloudflare-activate.sh --cleanup-docker-cloudflared
```
...
`frontend/modular/scripts/srp-public-acceptance.mjs` now includes nested TLS error details. On this Windows Node environment, public `fetch` reports:
