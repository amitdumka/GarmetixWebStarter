...
| --- | --- | --- |
| Legacy frontend root | `frontend/legacy/garmetix-web` | Existing Nuxt app. This is the source for route extraction. |
| New modular frontend root | `modular` | Stage 12A shell workspace with six Nuxt app shells and shared package folders. |
| Backend/API root | `backend/Garmetix.Api` | ASP.NET Core Web API. Keep as one API for Stage 12. |
| Domain project | `backend/Garmetix.Domain` | Shared domain models and generated models. |
| Infrastructure project | `backend/Garmetix.Infrastructure` | EF Core DbContext, migrations, repository wiring. |
| Tests | `backend/Garmetix.Api.Tests` | Existing backend test project. |
...
| Legacy Nuxt version | Nuxt `^4.4.8` | From `frontend/legacy/garmetix-web/package.json`. |
...
Legacy config: `frontend/legacy/garmetix-web/nuxt.config.ts`
...
Routes are generated from `frontend/legacy/garmetix-web/pages`.
...
Menu source: `frontend/legacy/garmetix-web/components/AppShell.vue`. `AppShellLegacy.vue` mirrors much of the same navigation for legacy shell mode.
...
Source: `frontend/legacy/garmetix-web/composables/useAuth.ts`
...
Primary API client: `frontend/legacy/garmetix-web/composables/useGarmetixApi.ts`
...
Source: `frontend/legacy/garmetix-web/composables/useAccessControl.ts`
...
- `frontend/legacy/garmetix-web/Dockerfile`
- `backend/Garmetix.Api/Dockerfile`
...
| --- | --- | --- |
| API client | `frontend/legacy/garmetix-web/composables/useGarmetixApi.ts` | `frontend/modular/packages/shared-api` |
| Auth helper | `frontend/legacy/garmetix-web/composables/useAuth.ts` | `frontend/modular/packages/shared-auth` |
| Permission helper | `frontend/legacy/garmetix-web/composables/useAccessControl.ts` | `frontend/modular/packages/shared-auth` or `frontend/modular/packages/shared-types` plus app adapters |
| Workspace helper | `frontend/legacy/garmetix-web/composables/useWorkspace.ts` | `frontend/modular/packages/shared-auth` or `frontend/modular/packages/shared-api` |
| Common types/interfaces | Inline composable/page types and generated DTO shapes | `frontend/modular/packages/shared-types` |
| Date/money formatting | Currently scattered in pages/components | `frontend/modular/packages/shared-utils` |
...
| Basic UI components | `components/ui/*`, dashboard cards/tables, entry forms where generic | `frontend/modular/packages/shared-ui` |
| App registry/menu metadata | `AppShell.vue` module groups and `useAccessControl.ts` rules | `frontend/modular/packages/shared-types` or `frontend/modular/config/routes.ts` |
...
1. Keep the shared route registry in `frontend/modular/config/routes.ts` as the source of truth for modular ownership and app links.
2. Wire route/menu helpers into shared shell UI so each modular app shows only its owned routes.
3. Extract `useAuth` session types and token helpers into `frontend/modular/packages/shared-auth` without changing legacy behavior.
4. Extract `useGarmetixApi` request logic into `frontend/modular/packages/shared-api`, keeping auth headers injectable so every app can reuse it.
