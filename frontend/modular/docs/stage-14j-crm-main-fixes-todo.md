...
## Note on the earlier session's unresolved deploy-tooling bug (RESOLVED)

The Git-Bash `dotnet publish` doubled-drive-letter bug (`C:\c\AIArea\...`) is now **fixed and verified** (commit `99a835d`, supersedes the insufficient first attempt `22988f0`). Root cause: Git Bash/MSYS's own implicit argv-to-Windows-path conversion for the native `dotnet` executable corrupts the path — not the `wslpath` logic the first fix targeted (live tracing proved `wslpath` was never even on PATH and the old `.exe`-suffix guard never matched). Fix: `dotnet_path_arg()` now explicitly calls `cygpath -w` under Git Bash/MSYS, reserving `wslpath -w` for genuine WSL only. Verified against a real `--apps=hr --build-only` run producing a correct, complete API publish output. The Attendance Policies `DELETE` endpoint and the DigitalInvoice cleanup fix are both still not live on SRP — they're committed to git but require an actual deploy, which is still on hold per the 3-iteration gate.
