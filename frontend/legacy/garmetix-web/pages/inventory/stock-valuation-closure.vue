...
useHead({ title: 'Stock Valuation Closure | Garmetix' })
...
const primaryRows = computed(() => report.value?.rows || [])
const secondaryRows = computed(() => report.value?.summary || [])
...
async function refreshShell() {
  ...
  try {
    ...
    loadError.value ||= feedback.errorMessage(error, 'Workspace options could not be loaded.', 'Stock Valuation Closure workspace load failed')
  }
}
async function loadReport() {
  ...
  loadError.value = ''
  try {
    report.value = await api.get<any>(`inventory/stock-valuation-closure?${query()}`)
    feedback.notify('Stock Valuation Closure refreshed', `${criticalIssues.value} critical and ${warningIssues.value} warning issue(s).`, report.value?.status === 'Complete' ? 'success' : 'warning')
  } catch (error: any) {
    loadError.value = feedback.errorMessage(error, 'Stock Valuation Closure could not be loaded. Check API logs and permissions.', 'Stock Valuation Closure failed')
...
async function exportCsv() {
  ...
  try {
    const response = await fetch(`${config.public.apiBase}/inventory/stock-valuation-closure/evidence.csv?${query()}`, { headers: api.authHeaders() })
    if (!response.ok) throw new Error(await response.text())
    const blob = await response.blob()
    const url = URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = `stock-valuation-closure-${filters.from || filters.asOf}-${filters.to || filters.asOf}.csv`
    link.click()
    URL.revokeObjectURL(url)
  } catch (error: any) { feedback.fromError(error, 'Stock Valuation Closure CSV export failed') }
...
<template>
  ...
  <AppShell v-else title="Stock Valuation" :companies="companies" :stores="stores" @refresh="refreshAll" @workspace-change="loadReport">
    <section class="closure-page">
      <UiModulePageHeader title="Stock Valuation Closure" description="Inventory valuation acceptance for current stock, cost price, negative rows, movement proof and CSV evidence before month/FY close." icon="i-lucide-boxes">
        <template #actions>
      ...
      <UCard class="planner-card">
        <div class="validation-filters">
          <UFormField label="As of date"><UInput v-model="filters.asOf" type="date" /></UFormField>
      ...
      <UCard class="section-card">
        <template #header><h3>Stock valuation rows</h3></template>
        <div class="table-wrap"><table><thead><tr><th>Product</th><th>Barcode</th><th>Current Qty</th><th>Cost</th><th>MRP</th><th>Cost Value</th><th>Movement Rows</th><th>Status</th></tr></thead><tbody><tr v-for="row in primaryRows" :key="row.id || row.invoiceId || row.stockId || row.module"><td>{{ row.productName }}</td><td>{{ row.barcode }}</td><td>{{ numberValue(row.currentQty) }}</td><td>{{ money(row.costPrice) }}</td><td>{{ money(row.mrp) }}</td><td>{{ money(row.costValue) }}</td><td>{{ numberValue(row.movementRows) }}</td><td><UBadge :label="row.status" :color="statusColor(row.status)" variant="subtle" /></td></tr></tbody></table></div>
      </UCard>

      <UCard v-if="secondaryRows.length" class="section-card">
        <template #header><h3>Stock-type summary</h3></template>
        <div class="table-wrap"><table><thead><tr><th>Stock Type</th><th>Rows</th><th>Quantity</th><th>Cost Value</th><th>MRP Value</th></tr></thead><tbody><tr v-for="row in secondaryRows" :key="row.id || row.productId || `${row.invoiceId}-${row.barcode}`"><td>{{ row.stockType }}</td><td>{{ numberValue(row.rows) }}</td><td>{{ numberValue(row.quantity) }}</td><td>{{ money(row.costValue) }}</td><td>{{ money(row.mrpValue) }}</td></tr></tbody></table></div>
      </UCard>
