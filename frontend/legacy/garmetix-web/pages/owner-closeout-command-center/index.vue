...
useHead({ title: 'Owner Closeout Command Center | Garmetix' })
...
const primaryRows = computed(() => report.value?.modules || [])
...
async function refreshShell() {
  ...
  try {
    ...
    loadError.value ||= feedback.errorMessage(error, 'Workspace options could not be loaded.', 'Owner Closeout Command Center workspace load failed')
  }
}
async function loadReport() {
  ...
  loadError.value = ''
  try {
    report.value = await api.get<any>(`owner-closeout-command-center?${query()}`)
    feedback.notify('Owner Closeout Command Center refreshed', `${criticalIssues.value} critical and ${warningIssues.value} warning issue(s).`, report.value?.status === 'Complete' ? 'success' : 'warning')
  } catch (error: any) {
    loadError.value = feedback.errorMessage(error, 'Owner Closeout Command Center could not be loaded. Check API logs and permissions.', 'Owner Closeout Command Center failed')
...
async function exportCsv() {
  ...
  try {
    const response = await fetch(`${config.public.apiBase}/owner-closeout-command-center/evidence.csv?${query()}`, { headers: api.authHeaders() })
    if (!response.ok) throw new Error(await response.text())
    const blob = await response.blob()
    const url = URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = `owner-closeout-command-center-${filters.from || filters.asOf}-${filters.to || filters.asOf}.csv`
    link.click()
    URL.revokeObjectURL(url)
  } catch (error: any) { feedback.fromError(error, 'Owner Closeout Command Center CSV export failed') }
...
<template>
  ...
  <AppShell v-else title="Owner Closeout" :companies="companies" :stores="stores" @refresh="refreshAll" @workspace-change="loadReport">
    <section class="closure-page">
      <UiModulePageHeader title="Owner Closeout Command Center" description="One owner/accountant control page combining profit, stock valuation, dues, payables, bank settlement and accounting balance evidence." icon="i-lucide-layout-dashboard">
        <template #actions>
      ...
      <UCard class="section-card">
        <template #header><h3>Module closeout matrix</h3></template>
        <div class="table-wrap"><table><thead><tr><th>Module</th><th>Status</th><th>Primary</th><th>Secondary</th><th>Issues</th><th>Description</th><th>Open</th></tr></thead><tbody><tr v-for="row in primaryRows" :key="row.id || row.invoiceId || row.stockId || row.module"><td>{{ row.module }}</td><td><UBadge :label="row.status" :color="statusColor(row.status)" variant="subtle" /></td><td>{{ row.primaryAmount !== null && row.primaryAmount !== undefined ? money(row.primaryAmount) : '-' }}</td><td>{{ row.secondaryAmount !== null && row.secondaryAmount !== undefined ? money(row.secondaryAmount) : '-' }}</td><td>{{ numberValue(row.issueCount) }}</td><td>{{ row.description }}</td><td><UButton :to="row.actionPath" size="xs" variant="ghost" icon="i-lucide-external-link" label="Open" /></td></tr></tbody></table></div>
      </UCard>

      <UCard v-if="secondaryRows.length" class="section-card">
        <template #header><h3>Not used</h3></template>
        <div class="table-wrap"><table><thead><tr><th></th></tr></thead><tbody><tr v-for="row in secondaryRows" :key="row.id || row.productId || `${row.invoiceId}-${row.barcode}`"><td></td></tr></tbody></table></div>
      </UCard>
