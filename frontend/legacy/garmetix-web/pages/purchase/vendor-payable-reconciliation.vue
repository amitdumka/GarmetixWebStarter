...
useHead({ title: 'Vendor Payable Reconciliation | Garmetix' })
...
const paymentSources = computed(() => report.value?.paymentSources || [])
...
async function refreshShell() {
  ...
  try {
    ...
    loadError.value ||= feedback.errorMessage(error, 'Workspace options could not be loaded.', 'Vendor payable reconciliation workspace load failed')
  }
}
...
async function loadReport() {
  ...
  loadError.value = ''
  try {
    report.value = await api.get<any>(`purchase/vendor-payable-reconciliation?${query()}`)
    const status = report.value?.status || 'Not Complete'
    feedback.notify('Vendor payable reconciliation refreshed', `${criticalIssues.value} critical and ${warningIssues.value} warning issue(s).`, status === 'Complete' ? 'success' : 'warning')
  } catch (error: any) {
    loadError.value = feedback.errorMessage(error, 'Vendor payable reconciliation could not be loaded. Check API logs and permissions.', 'Vendor payable reconciliation failed')
  } finally {
    loading.value = false
  }
}
...
async function exportCsv() {
  ...
  try {
    const response = await fetch(`${config.public.apiBase}/purchase/vendor-payable-reconciliation/evidence.csv?${query()}`, { headers: api.authHeaders() })
    if (!response.ok) throw new Error(await response.text())
    const blob = await response.blob()
    const url = URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = `vendor-payable-reconciliation-${filters.from}-${filters.to}.csv`
    link.click()
    URL.revokeObjectURL(url)
  } catch (error: any) {
    feedback.fromError(error, 'Vendor payable reconciliation CSV export failed')
  } finally {
...
<template>
  ...
  <AppShell
    v-else
    title="Vendor Payable Reco"
    :companies="companies"
    :stores="stores"
    @refresh="refreshAll"
    @workspace-change="loadReport"
  >
    <section class="vendor-payable-reco-page">
      <UiModulePageHeader
        title="Vendor payable / purchase settlement reconciliation"
        description="Validate supplier invoice dues, vendor master bill/paid totals, payment vouchers, non-cash bank mapping, debit notes and vendor settlements before payable closeout."
      ...
      >
        <template #actions>
          <div class="header-actions">
            ...
            <UButton to="/vendor-payments" icon="i-lucide-hand-coins" label="Vendor Payments" variant="ghost" />
          </div>
        </template>
      </UiModulePageHeader>
      ...
      <UAlert
        ...
        title="Vendor payable reconciliation is unavailable"
      ...
      <div class="status-grid">
        <UCard class="status-card">
          ...
          <p class="muted">Complete only when vendor payable, payment, debit-note and settlement evidence has no critical or warning issue.</p>
        </UCard>
        <UCard class="status-card">
          <p class="eyebrow">Critical</p>
          <strong>{{ formatNumber(criticalIssues) }}</strong>
          <p class="muted">Blocks payable closeout.</p>
        </UCard>
        <UCard class="status-card">
      ...
      <UCard>
        <template #header>
          <div class="card-header-line">
            <div>
              <h2>Payment / settlement source reconciliation</h2>
              <p>Shows invoice payments, advances, debit-note settlements and supplier refund settlement evidence.</p>
            </div>
          </div>
        </template>
        <div class="planner-table-wrap">
          <table class="planner-table">
            <thead><tr><th>Source</th><th class="text-right">Total rows</th><th class="text-right">Total amount</th><th class="text-right">Period rows</th><th class="text-right">Period amount</th></tr></thead>
            <tbody>
              <tr v-for="source in paymentSources" :key="source.source">
                <td class="font-medium">{{ source.source }}</td>
                <td class="text-right">{{ formatNumber(source.totalRows) }}</td>
                <td class="text-right">{{ money(source.totalAmount) }}</td>
                <td class="text-right">{{ formatNumber(source.periodRows) }}</td>
                <td class="text-right">{{ money(source.periodAmount) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <template #header>
          <div class="card-header-line">
            <div>
              ...
              <p>Correct these through purchase invoice, vendor payment, debit-note settlement, voucher/accounting repair or data consistency repair.</p>
            </div>
          </div>
        </template>
        <div class="planner-table-wrap">
          <table class="planner-table">
            <thead><tr><th>Severity</th><th>Code</th><th>Vendor</th><th>Mobile</th><th>GSTIN</th><th>Message</th><th class="text-right">Amount</th></tr></thead>
            <tbody>
              <tr v-for="issue in issues" :key="`${issue.vendorId}-${issue.code}-${issue.message}`">
                ...
                <td class="font-medium">{{ issue.vendorName }}</td>
                <td>{{ issue.mobileNumber || '-' }}</td>
                <td>{{ issue.gstin || '-' }}</td>
                <td>{{ issue.message }}</td>
                <td class="text-right">{{ issue.amount === null || issue.amount === undefined ? '-' : money(issue.amount) }}</td>
              </tr>
              <tr v-if="issues.length === 0"><td colspan="7" class="empty-row">No issues for the selected close date.</td></tr>
            </tbody>
          </table>
        </div>
      </UCard>
      ...
      <UCard>
        ...
        <div class="planner-table-wrap wide-table">
          <table class="planner-table">
            <thead>
              <tr>
                <th>Status</th><th>Vendor</th><th>Mobile</th><th>GSTIN</th><th class="text-right">Invoices</th><th class="text-right">Open</th><th class="text-right">Invoice Total</th><th class="text-right">Payments</th><th class="text-right">Outstanding</th><th class="text-right">Master Bill</th><th class="text-right">Master Paid</th><th class="text-right">Open DN</th><th class="text-right">Expected Payable</th><th class="text-right">Diff</th><th class="text-right">Voucher Issues</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="row in evidenceRows" :key="row.vendorId">
                ...
                <td class="text-right">{{ money(row.invoiceTotal) }}</td>
                <td class="text-right">{{ money(row.activePaymentRows) }}</td>
                <td class="text-right">{{ money(row.invoiceOutstanding) }}</td>
                <td class="text-right">{{ money(row.masterBillAmount) }}</td>
                <td class="text-right">{{ money(row.masterPaidAmount) }}</td>
                <td class="text-right">{{ money(row.openDebitNoteAmount) }}</td>
                <td class="text-right">{{ money(row.expectedPayableBalance) }}</td>
                <td class="text-right">{{ money(row.balanceDifference) }}</td>
                <td class="text-right">{{ formatNumber(row.voucherIssueCount) }}</td>
              </tr>
              <tr v-if="evidenceRows.length === 0"><td colspan="15" class="empty-row">No vendor payable evidence found for this filter.</td></tr>
            </tbody>
          </table>
        </div>
      </UCard>
...
<style scoped>
.vendor-payable-reco-page { display: grid; gap: 1rem; }
