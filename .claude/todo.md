...
- [ ] **Deploy script `dotnet publish` Git-Bash path bug (found 2026-07-07, not fixed)**: `srp-whole-site-deploy.sh`'s dotnet publish step fails with a doubled drive-letter path (`C:\c\AIArea\...`) specifically when the script runs via Git Bash (`GARMETIX_PREFER_WSL=false`) rather than WSL. Worked around via `--skip-api` since no backend changes existed that session; needs a real fix (likely a path-conversion issue between MSYS bash and the native dotnet.exe) before Git-Bash-based deploys can safely republish the API.
