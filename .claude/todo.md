...
- [ ] **Import Acceptance QA dashboard** (`/purchase/import-acceptance`, `pages/purchase/import-acceptance.vue`) - posting-report/backup-restore/acceptance checklist dashboard for the import batches above (`acceptance-summary`, `parser-qa-summary`, `final-closure-status`, `backup-restore-checklist`, `storage-summary` endpoints - none wired yet).
- [ ] **Import Learning profiles** (`/purchase/import-profiles`, `pages/purchase/import-profiles.vue`) - per-vendor OCR alias/ignored-pattern management (`vendor-profiles` CRUD endpoints - none wired yet).
- [ ] **Import secondary features not wired in the core workflow pass**: audit-json/lines-csv per-batch export, cleanup-history (bulk draft deletion), acceptance status update, correction-safety/correction-plan/request-correction (the legacy "audit trail" workflow for already-posted batches), multi-file listing/download (only the single proof file is wired).
