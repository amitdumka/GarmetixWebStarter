...
## 2026-07-17 - Swalekha PersonalFin_06 (Multi-Owner Data Isolation) - branch `swalekha`

**Type**: Foundational security/privacy fix on the Swalekha module, inserted ahead of the next feature stage. No deploy executed.

**What happened**: Amit asked to go ahead with the next part, and separately flagged a real design gap: "data if will be profile based, Suppose, In a Company two owner is there then each owner will have their own data, Login id decide the owner profile." Correct catch - Swalekha's five stages so far had zero concept of "which Owner." Every table (`SwalekhaAccount`, `SwalekhaContact`, `SwalekhaExpenseSheet`, etc.) was a flat, unscoped list - if two `UserType.Owner` logins ever used the platform, they would have silently shared the exact same accounts, contacts, expenses, everything. This is exactly the kind of gap that's dangerous precisely because it's invisible in single-owner testing (which is all this module had seen so far) and only surfaces once a second real owner logs in. Fixed as its own stage, `PersonalFin_06`, before building anything else on top of the insecure foundation.

**Design choice - global query filter over per-endpoint filtering**: considered the simpler-looking option (thread an `ownerId` parameter into every endpoint handler and add `.Where(x => x.OwnerId == ownerId)` by hand, matching how `CompanyId`/`StoreId` scoping is actually done in the main `GarmetixDbContext` - confirmed via research that the main context has no global tenant filter, only explicit per-endpoint scoping via `WorkspaceScope.ApplyTo`). Rejected it for Swalekha specifically: financial data leaking between owners is a genuine privacy failure, not a cosmetic bug, and "remember to add the filter" is exactly the kind of thing that's easy to forget on the 35th endpoint handler across 6 files. Went with EF Core's standard "current tenant" global-query-filter pattern instead - a single choke point that can't be bypassed by an endpoint author simply forgetting a `.Where()` clause. The main `GarmetixDbContext`'s own choice not to do this for Company/Store scoping is a separate, already-established decision for that context; Swalekha getting its own stricter guarantee here doesn't need to match it.

**Backend**:
- `backend/Garmetix.Domain/Generated/Models/Swalekha/SwalekhaOwnedEntity.cs` - new `SwalekhaOwnedEntity : BaseEntity { Guid OwnerId }`. All 9 existing entities (`SwalekhaAccount`, `SwalekhaAccountTransaction`, `SwalekhaContact`, `SwalekhaPersonLedgerEntry`, `SwalekhaExpenseSheet`, `SwalekhaExpenseEntry`, `SwalekhaIncomeEntry`, `SwalekhaRecurringBill`, `SwalekhaTrip`) retrofitted to extend it instead of plain `BaseEntity`.
- `backend/Garmetix.Infrastructure/Data/SwalekhaOwnerContext.cs` (new) + `backend/Garmetix.Api/Swalekha/SwalekhaOwnerMiddleware.cs` (new) - a scoped POCO populated per-request from the JWT's `NameIdentifier` claim, mirroring the codebase's existing `AuditActorContext`/`AuditActorMiddleware` pattern exactly (confirmed via research this is the actual established idiom here, not `IHttpContextAccessor`, which exists in DI but is essentially unused outside one Assistant MCP feature). Registered `AddScoped<SwalekhaOwnerContext>()` and `app.UseMiddleware<SwalekhaOwnerMiddleware>()` (right after the existing `AuditActorMiddleware`, same position in the pipeline - after `UseAuthentication()` so `context.User` is already populated, before `UseAuthorization()`).
- `SwalekhaDbContext` - constructor now takes an optional `SwalekhaOwnerContext? ownerContext = null` (same `= null` default pattern `GarmetixDbContext` already uses for `AuditActorContext?`, so design-time tooling still works). New `CurrentOwnerId` property. `OnModelCreating` now builds a combined `!Deleted && OwnerId == CurrentOwnerId` filter for every `SwalekhaOwnedEntity` type (`CreateOwnedFilter`, extending the exact same generic expression-tree-per-entity-type technique the existing `CreateSoftDeleteFilter` already used - just adding one more `AndAlso` condition referencing `this.CurrentOwnerId`, the standard EF Core "current tenant" global-filter idiom). New `SaveChanges`/`SaveChangesAsync` overrides auto-stamp `OwnerId` on every `Added` entry - but **only when it's still `Guid.Empty`**, deliberately never overwriting an already-set value. That one carve-out is exactly what `PersonalFin_07`'s family-transfer sync will need: a handler can explicitly set a *different* (recipient) `OwnerId` before saving, to post a row directly into someone else's data on their behalf, without the auto-stamp clobbering it back to the current caller.
- `SwalekhaSchemaRepairService.cs` - one trailing block of `ALTER TABLE ... ADD COLUMN IF NOT EXISTS "OwnerId"` + `CREATE INDEX` across all 9 tables (order-independent relative to each table's own `CREATE TABLE`/column statements above it, so no need to interleave edits into 9 separate spots).

**Verified live with two real Owner logins** (continuing the discipline established in the prior local-testing session, not just build-checked): created a second Owner test user (`swalekha_owner2`) via the existing `POST /api/access/users` endpoint, then made direct authenticated API calls as both users. Confirmed: Owner 2's account list starts empty; Owner 2 creating an account is completely invisible to Owner 1's `GET /accounts`; Owner 1's list stays exactly as it was. The core privacy guarantee holds. **Pre-existing test data from the earlier local-testing session (created before this fix existed) is now correctly "orphaned"** - the `ALTER TABLE ... DEFAULT '00000000-0000-0000-0000-000000000000'` gives old rows an `OwnerId` that matches no real user, so they've dropped out of every owner's view. This is expected and safe (it was disposable test data, not real financial records), and is exactly the correct behavior for data that predates ownership tracking - flagged here for the record rather than silently glossed over.

**Validated**: `dotnet build` (0 errors, same 7 pre-existing unrelated warnings - had to stop the locally-running backend process first since it held the output DLLs locked from a previous test session, a purely mechanical blocker not a code issue), full backend test suite (281 passed, 3 pre-existing Postgres-only skipped, zero regressions). No frontend changes this stage - the isolation is entirely server-side and transparent to every existing page. Version bumped to `6.9.12`. Roadmap renumbered: the former `PersonalFin_06`-`13` (Investments/Loans/Insurance/Organizer/Dashboard/Vault) are now `PersonalFin_08`-`15`, with `PersonalFin_07` (Owner Profile + Family Connections + Transaction Sync) inserted next.

**Next**: `PersonalFin_07` (Owner Profile + Family Connections + Transaction Sync + Employee-table auto-provisioning).
...
**What happened**: Amit asked to run the Swalekha module locally for testing. This required real infrastructure every prior Swalekha stage's validation had explicitly lacked: a live Postgres database and a real Owner login (every stage 1-5 changelog entry ends with "no live click-through possible - no test credentials in this environment"). Set it up from scratch:

- Docker Desktop wasn't running; started it, found an existing `legacy-postgres-1` container (from an earlier session) already healthy on port 5432 with the `garmetix` database populated.
- Started the backend API locally (`ASPNETCORE_ENVIRONMENT=Development`, pointed at `localhost:5432` via `appsettings.Development.json`) - **first real gotcha**: running `dotnet Garmetix.Api.dll` from the repo root failed with "Connection string 'Default' is missing" even though the file had the right content, because ASP.NET Core resolves `appsettings.json` relative to the process's *current working directory*, not the DLL's location - fixed by `cd`-ing into the build output directory first before running.
- On startup, confirmed via the live logs that `swalekha_db` and every `Swalekha*` table were created for the first time ever outside a build/test run, and the seeded SuperAdmin (`garmetix`/`Garmetix@1234!`, from `SystemDefaultsService`) was present.
- Logged in as SuperAdmin, then used the existing `POST /api/access/users` endpoint (Admin-gated) to create a real `UserType.Owner` test login (`swalekha_owner`) - the only realistic path, since there's no public self-registration endpoint and the full client-onboarding flow (`POST /api/client-onboarding/submit`) is company-onboarding-shaped, overkill for a Swalekha-only test account which needs no Company/Store association at all. **Second real gotcha**: the request body's `role`/`userType`/`appOperation` enum fields had to be sent as their numeric ordinal values, not string names - `System.Text.Json` on this endpoint has no `JsonStringEnumConverter` registered, so `"role": "Member"` failed to deserialize while `"role": 5` worked.
- **Third real gotcha**: logging into the `swalekha-web` dev server (port 3109) against the backend failed with a browser-level "Failed to fetch" - the backend's CORS policy (`Cors:AllowedOrigins`) only allows `http://localhost:3000` by default, and none of the 9 business apps' dev ports (3100-3109) were in it either. Restarted the backend with `Cors__AllowedOriginsCsv` covering all local dev app ports - a local-only env var, not committed to any config file.

**With the stack actually running, clicked through every PersonalFin_02-05 feature with real data - and found two genuine, previously-undetected product bugs immediately:**

1. **`UTooltip` wrapping a `UButton` inside a `UTable` `#actions-cell` slot throws at render time** ("Unhandled error during execution of setup function", visible only as a Vue console warning) - this silently kills that row's rendering entirely. The Accounts Hub table showed its header row and nothing else, even though the summary cards (reading the exact same `accounts` ref) correctly showed 1 account worth ₹50,000 - there was no visible error to the user, it just looked like an empty list. This exact pattern (`<UTooltip text="X"><UButton .../></UTooltip>` inside every list page's actions column) was used in **all 10** Swalekha page files, so it affected every single list in the module. Fixed by replacing every instance with a plain `<UButton ... title="X" />` (native browser tooltip, zero dependency on the Nuxt UI Tooltip primitive). Verified live: the exact same account row rendered correctly immediately after the fix, with zero console errors.
2. **Trip creation/update failed with a 500** whenever the Start/End date fields were left blank (the common case - a trip doesn't need a date range to start tracking expenses against). `SwalekhaTripPayload.StartDate`/`EndDate` are `DateTime?` on the backend; the frontend's `<UInput type="date">` uses `''` as its native "empty" sentinel, and `System.Text.Json` cannot deserialize an empty string into `DateTime?` (only a real ISO date or a JSON `null`) - every submission threw `JsonException: The JSON value could not be converted... Path: $.startDate`. Fixed in `submitTrip()` by converting empty-string dates to `null` before posting. Verified live: trip creation, its own auto-created Travel-typed expense sheet, an expense entry against it, and Close Trip all worked correctly afterward.

**Every feature was then genuinely exercised end to end for the first time**, not just build-checked: created a Bank account and a Cash account, posted a deposit (balance updated correctly), transferred ₹500 between them (both balances updated, net worth unchanged - confirming the linked-transaction-pair logic from `PersonalFin_02` works); created a contact, posted a Loan Given entry, used Settle (correctly auto-computed and posted the exact repayment needed to zero the balance); created an expense sheet with a budget, posted a visible entry and a Hidden entry (confirmed the hidden entry is excluded from the default list but counted in the sheet total, and appears when "Show hidden" is toggled - exactly as designed); posted an income entry; created a recurring bill and used Mark Paid (badge flipped from "Due" to "Paid <date>"); created a trip, posted an expense against its auto-created Travel sheet, and closed it - then confirmed on the Expenses summary page that the Travel category total picked up the trip's spend automatically with zero manual roll-up step, exactly matching the `PersonalFin_05` design rationale.

**Follow-up flagged, not fixed in this session**: the `UTooltip`-in-`UTable`-cell pattern that broke Swalekha was originally copied from `frontend/modular/apps/final-accounts/pages/chart-of-accounts.vue`, which uses the identical pattern and - per this same project's own repeated "no live click-through possible" history - has likely never been tested against real data either. Spawned a background task (`task_9f4f692c`) to audit and fix the same pattern across the rest of the modular frontend.

**Validated**: clean `swalekha-web` production build after both fixes, `node scripts/validate-structure.mjs` passing, and (unlike every prior stage) genuine authenticated click-through against a live backend + database covering every `PersonalFin_02`-`05` feature with zero console errors remaining. No backend changes were needed - both bugs were frontend-only. Version bumped to `6.9.10`.
