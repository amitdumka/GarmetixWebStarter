...
## Swalekha (Personal & Personal Finance) Module - branch `swalekha` is FINAL (tag `swalekha-final-v6.9.21`), not yet deployed

**Branch closed 2026-07-17.** Amit confirmed no further commits are planned on `swalekha` - the final commit (`33c6543`, the dedicated SRP deploy script) is tagged `swalekha-final-v6.9.21`. Any future Swalekha work (bug fix, new stage, or the actual deploy) should branch again from this point rather than assume `swalekha` keeps moving.

A brand-new, completely isolated module (Owner-login-only, own `swalekha_db` database, own frontend app not registered in the app switcher/sidebar) covering personal finance (accounts/loans/investments/insurance/expenses/travel sheets/person-to-person ledger) and a personal organizer (diary/notes/calendar/contacts). Full design, naming, isolation architecture and the 15-stage plan (`PersonalFin_01`..`PersonalFin_15`) are in `docs/personal-finance-module-design.md` and `.claude/todo.md`. All work happened on the dedicated `swalekha` branch (created 2026-07-17 from `version6`), committed and pushed stage by stage per Amit's "keep going and keep pushing to each stage, complete it" - `version6` itself stays untouched. **`PersonalFin_01` through `PersonalFin_15` are all done (v6.9.21) - the entire original roadmap is code-complete**, plus a dedicated `srp-swalekha-deploy.sh` script that deploys/configures the module onto an SRP host that already has the rest of Garmetix installed (validated locally via `--dry-run`/`--build-only`, not run against the live host). `PersonalFin_06` (multi-owner data isolation) and `PersonalFin_07` (Owner Profile + Family Connections + a mutual-consent "one entry syncs both sides" transfer, incl. auto-provisioning from the Employee table) were both inserted after Amit's follow-up ask exposed a real per-Owner data gap and asked for family-linked accounts. The full Investments trio - `PersonalFin_08` (FD/RD), `PersonalFin_09` (Mutual Funds/SIP, incl. a standalone unit-tested XIRR calculator), `PersonalFin_10` (Shares/Stocks with realized+unrealized P&L, plus simple PPF/EPF/NPS/Gold snapshots) - is done, all genuinely integrated with the Accounts Hub ledger. `PersonalFin_11` (Loans Taken, incl. a standalone unit-tested EMI/amortization calculator) and `PersonalFin_12` (Insurance - premium reminders, maturity payouts) closed out Pillar A. `PersonalFin_13` (Diary/Journal, Personal Notes, Calendar with live-merged finance due-dates) started Pillar B. `PersonalFin_14` (Dashboard & Reports) rebuilt the Dashboard to aggregate a genuine Net Worth across every entity built so far, a rolling 30-day Upcoming Dues feed, and a CSV Net Worth export - live-verified against real accumulated test data, matching to the rupee. `PersonalFin_15` (Document Vault & Hardening, the final stage) added a Document Vault (upload/download/delete scanned proofs against any entity) and a live runtime Owner-isolation self-check (proved against real other-Owner data, not an empty table), plus extended the `swalekha_db` backup coverage into `srp-backup-database.sh`. A small About Swalekha page credits AKS Labs (India) / Amit Kumar. **Swalekha remains undeployed** - deployment is Amit's call, to be made once he's ready to review the whole module together.
