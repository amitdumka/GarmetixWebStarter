...
## Scheduled overnight run (2026-07-11 04:30 IST)

A one-shot scheduled task (`purchase-import-tailoring-overnight`) is queued to finish the Purchase Import satellite pages (Import Acceptance QA, Import Learning/vendor-profiles - both backend-complete, frontend-only work), port the Tailoring & Alteration module (backend-complete, `/tailoring` is currently just a placeholder stub in modular), and - as the one schema-touching piece - add a `VendorType` column to the existing `Vendor` entity so tailoring vendors can be genuinely distinguished from purchase vendors (an orphaned `VendorType` enum with a `Tailoring` literal already exists but isn't wired to anything). See `.claude/todo.md` for the itemized v6.2.1-6.2.4 breakdown and the exact backend file/line references the scheduled run was briefed with, so the next session can pick this up if the overnight run stops partway.
