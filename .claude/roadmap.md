...
A brand-new, completely isolated module (Owner-login-only, own `swalekha_db` database, own frontend app not registered in the app switcher/sidebar) covering personal finance (accounts/loans/investments/insurance/expenses/travel sheets/person-to-person ledger) and a personal organizer (diary/notes/calendar/contacts). Full design, naming, isolation architecture and a 13-stage plan (`PersonalFin_01`..`PersonalFin_13`) are in `docs/personal-finance-module-design.md` and `.claude/todo.md`. All work happens on the dedicated `swalekha` branch (created 2026-07-17 from `version6`), committed and pushed stage by stage per Amit's "keep going and keep pushing to each stage" - `version6` itself stays untouched. `PersonalFin_01` through `PersonalFin_13` are done (v6.9.19) - all of Pillar A (Personal Finance) plus the start of Pillar B (Personal Organizer). `PersonalFin_06` (multi-owner data isolation) and `PersonalFin_07` (Owner Profile + Family Connections + a mutual-consent "one entry syncs both sides" transfer, incl. auto-provisioning from the Employee table) were both inserted after Amit's follow-up ask exposed a real per-Owner data gap and asked for family-linked accounts. The full Investments trio - `PersonalFin_08` (FD/RD), `PersonalFin_09` (Mutual Funds/SIP, incl. a standalone unit-tested XIRR calculator), `PersonalFin_10` (Shares/Stocks with realized+unrealized P&L, plus simple PPF/EPF/NPS/Gold snapshots) - is done, all genuinely integrated with the Accounts Hub ledger. `PersonalFin_11` (Loans Taken, incl. a standalone unit-tested EMI/amortization calculator; "Loans Given" deliberately reuses `PersonalFin_03`'s Person Ledger rather than a second entity) and `PersonalFin_12` (Insurance - premium reminders via a frequency-aware due-date calculation, maturity payouts) closed out Pillar A. `PersonalFin_13` (Personal Organizer - Diary/Journal, Personal Notes, and a Calendar that merges real appointments with finance due-dates computed live from Pillar A) starts Pillar B. A small About Swalekha page credits AKS Labs (India) / Amit Kumar. `PersonalFin_14` (Dashboard & Reports - unified net-worth home, portfolio allocation, expense breakdown, Excel/PDF export) is next.
