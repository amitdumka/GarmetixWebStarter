...
## Swalekha (Personal & Personal Finance) Module - build in progress on branch `swalekha`

A brand-new, completely isolated module (Owner-login-only, own `swalekha_db` database, own frontend app not registered in the app switcher/sidebar) covering personal finance (accounts/loans/investments/insurance/expenses/travel sheets/person-to-person ledger) and a personal organizer (diary/notes/calendar/contacts). Full design, naming, isolation architecture and a 13-stage plan (`PersonalFin_01`..`PersonalFin_13`) are in `docs/personal-finance-module-design.md` and `.claude/todo.md`. All work happens on the dedicated `swalekha` branch (created 2026-07-17 from `version6`), committed and pushed stage by stage per Amit's "keep going and keep pushing to each stage" - `version6` itself stays untouched. `PersonalFin_01` through `PersonalFin_14` are done (v6.9.20) - all of Pillar A (Personal Finance), Pillar B started with the Personal Organizer, and the Dashboard is now the real "unified home" the design called for. `PersonalFin_06` (multi-owner data isolation) and `PersonalFin_07` (Owner Profile + Family Connections + a mutual-consent "one entry syncs both sides" transfer, incl. auto-provisioning from the Employee table) were both inserted after Amit's follow-up ask exposed a real per-Owner data gap and asked for family-linked accounts. The full Investments trio - `PersonalFin_08` (FD/RD), `PersonalFin_09` (Mutual Funds/SIP, incl. a standalone unit-tested XIRR calculator), `PersonalFin_10` (Shares/Stocks with realized+unrealized P&L, plus simple PPF/EPF/NPS/Gold snapshots) - is done, all genuinely integrated with the Accounts Hub ledger. `PersonalFin_11` (Loans Taken, incl. a standalone unit-tested EMI/amortization calculator) and `PersonalFin_12` (Insurance - premium reminders, maturity payouts) closed out Pillar A. `PersonalFin_13` (Diary/Journal, Personal Notes, Calendar with live-merged finance due-dates) started Pillar B. `PersonalFin_14` (Dashboard & Reports) rebuilt the Dashboard to aggregate a genuine Net Worth across every entity built so far, a rolling 30-day Upcoming Dues feed, and a CSV Net Worth export - **live-verified against real accumulated test data from every prior stage, matching to the rupee**. A small About Swalekha page credits AKS Labs (India) / Amit Kumar. `PersonalFin_15` (Document Vault & Hardening) is the last stage on the original roadmap.
