...
public static class CommercialEndpoints
{
    ...
    private static async Task<string> CreateNoteNumberAsync(Guid companyId, Guid storeId, NoteType noteType, GarmetixDbContext db, CancellationToken cancellationToken)
    {
        var prefix = noteType == NoteType.CreditNote ? "CN" : "DN";
        var today = DateTime.Today;
        var tomorrow = today.AddDays(1);
        var count = await db.CommercialNotes.CountAsync(item => item.CompanyId == companyId && item.StoreId == storeId && item.OnDate >= today && item.OnDate < tomorrow && item.NoteNumber.StartsWith(prefix + "-"), cancellationToken);
        return $"{prefix}-{today:yyyyMMdd}-{count + 1:0000}";
    }

    private static async Task<string> CreateAdvanceReceiptNumberAsync(Guid companyId, Guid storeId, GarmetixDbContext db, CancellationToken cancellationToken)
    {
        var today = DateTime.Today;
        var tomorrow = today.AddDays(1);
        var count = await db.CustomerAdvanceReceipts.CountAsync(item => item.CompanyId == companyId && item.StoreId == storeId && item.OnDate >= today && item.OnDate < tomorrow, cancellationToken);
        return $"ADV-{today:yyyyMMdd}-{count + 1:0000}";
    }
