...
curl -fsS "$API_URL/hr/employee-master/summary" -H "Authorization: Bearer $TOKEN" | python3 - <<'PY'
import json, sys
payload = json.load(sys.stdin)
required = ['totalEmployees','activeEmployees','missingPhoto','missingAadhaar','missingBank','readinessMessages']
missing = [k for k in required if k not in payload]
if missing:
    raise SystemExit(f'Missing employee summary keys: {missing}')
print('Employee master summary OK:', payload.get('totalEmployees'), 'employee(s)')
PY
curl -fsS -o /dev/null -w '%{http_code}\n' "$BASE_URL/hr" | grep -Eq '200|302|304'
echo "HR employee master acceptance drill passed."
