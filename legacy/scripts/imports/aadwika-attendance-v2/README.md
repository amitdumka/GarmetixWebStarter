...
Existing Attendance rows are updated by employee/date. New rows are inserted only when no active row exists for that employee/date. Punches created by this import batch are deleted and recreated. Manual punches are not deleted.
