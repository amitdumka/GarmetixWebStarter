...
SECRET_PATTERNS = [
    ...
    ('non-placeholder Cloudflare token assignment', re.compile(r"^[ \t]*CLOUDFLARE_API_TOKEN[ \t]*=[ \t]*(?!(?:['\"]?\$|CHANGE_ME|your_|$)).+", re.IGNORECASE | re.MULTILINE)),
    ('non-placeholder SSH password assignment', re.compile(r"^[ \t]*SSH_PASSWORD[ \t]*=[ \t]*(?!(?:['\"]?\$|CHANGE_ME|your_|$)).+", re.IGNORECASE | re.MULTILINE)),
    ('non-placeholder sudo password assignment', re.compile(r"^[ \t]*SUDO_PASSWORD[ \t]*=[ \t]*(?!(?:['\"]?\$|CHANGE_ME|your_|$)).+", re.IGNORECASE | re.MULTILINE)),
]
