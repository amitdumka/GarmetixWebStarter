...
const INVOICES = ['AF/2025/1345', 'AF/2025/1365', 'AF/2025/1380', 'AF/2025/1382', 'AF/2025/1386']
const SERVICE_INVOICE = 'AF/2025/1365'
...
const workbookPath = join(outputDir, 'batch-01-selected-invoices.xlsx')
const log = []
let token = ''
...
async function createServiceInvoice(salesman) {
  ...
  if (order) {
    ...
    order = await api('POST', 'tailoring/orders', {
      ...
      internalRemarks: `VyaparSourceInvoice=${SERVICE_INVOICE}; created by Codex batch-01 approved mapping.`,
  ...
  const updated = await api('PUT', `billing/sales/${converted.id}`, {
    ...
    customerGstin: null,
    salesmanId: salesman.id,
    remarks: `VyaparSaleImport; VyaparSourceInvoice=${SERVICE_INVOICE}; SourceDate=2026-02-20; Service invoice via Tailoring module; CodexBatch=batch-01.`
  })
...
writeJson('batch-01-preview-before-actions.json', preview)
log.push({
  action: 'preview',
...
writeJson('batch-01-confirm-invoices.json', confirmInvoices)
...
writeJson('batch-01-confirm-request-redacted.json', {
  ...confirmPayload,
...
writeJson('batch-01-confirm-result.json', confirmResult)
...
writeJson('batch-01-import-verification.json', verification)
for (const row of verification) log.push({ action: 'verify_imported', ...row })

writeJson('batch-01-action-log.json', log)
writeCsv('batch-01-action-log.csv', log)
...
writeFileSync(join(outputDir, 'batch-01-action-log.md'), `${md.join('\n')}\n`, 'utf8')

console.log(JSON.stringify({
  confirmResult,
  verification,
  logPath: join(outputDir, 'batch-01-action-log.md')
}, null, 2))
