...
const INVOICES = ['AF/2025/1162', 'AF/2025/1165']
const SERVICE_INVOICE = null

const lineActions = new Map([
  ['AF/2025/1162|222683372100M|Sparky-Shirt-3372/5104-Black,Blue,Cream,-M', {
    action: 'mapping_correct',
    overrideBarcode: '222683372100M',
    createProductAndStock: false,
    note: 'Amit approved existing mapping as correct.'
  }],
  ['AF/2025/1162|25105084|SS Seth Saab Jhodhpuri-LIMITED  EDITION-9121/79/CreamWhite-40', {
    action: 'mapping_correct',
    overrideBarcode: '25105084',
    createProductAndStock: false,
    note: 'Amit approved existing mapping as correct.'
  }],
  ['AF/2025/1162||20256-4/36/ALA/D595/INDO', {
    action: 'map_existing_barcode',
    overrideBarcode: '26058843',
    createProductAndStock: false,
    note: 'Amit approved mapping source 20256-4/36/ALA/D595/INDO to barcode 26058843.'
  }],
  ['AF/2025/1165|25012107|BlackJack-Blazer-BA225-B68/0425/2600-3-38', {
    action: 'mapping_correct',
    overrideBarcode: '25012107',
    createProductAndStock: false,
    note: 'Amit approved existing mapping as correct.'
  }],
  ['AF/2025/1165|23462E162100M|Sparky-Shirt-E182/5104-print-M', {
    action: 'mapping_correct',
    overrideBarcode: '23462E162100M',
    createProductAndStock: false,
    note: 'Amit approved existing mapping as correct.'
  }],
  ['AF/2025/1165||R240460373', {
    action: 'map_existing_barcode',
    overrideBarcode: 'R240460373',
    createProductAndStock: false,
    note: 'Amit approved mapping source R240460373 to barcode R240460373.'
  }],
  ['AF/2025/1165|ARVIND STORE|T190227030', {
    action: 'map_existing_barcode',
    overrideBarcode: 'T190227030',
    createProductAndStock: false,
    note: 'Amit approved mapping source T190227030 to barcode T190227030.'
  }]
])
...
const workbookPath = join(outputDir, 'batch-05-selected-invoices.xlsx')
const log = []
let token = ''
...
async function createServiceInvoice(salesman) {
  ...
  if (order) {
    ...
    order = await api('POST', 'tailoring/orders', {
      ...
      internalRemarks: `VyaparSourceInvoice=${SERVICE_INVOICE}; created by Codex batch-05 approved mapping.`,
  ...
  const updated = await api('PUT', `billing/sales/${converted.id}`, {
    ...
    customerGstin: null,
    salesmanId: salesman.id,
    remarks: `VyaparSaleImport; VyaparSourceInvoice=${SERVICE_INVOICE}; SourceDate=2026-02-20; Service invoice via Tailoring module; CodexBatch=batch-05.`
  })
...
writeJson('batch-05-preview-before-actions.json', preview)
log.push({
  action: 'preview',
...
writeJson('batch-05-confirm-invoices.json', confirmInvoices)
...
const confirmPayload = {
  ...
  paymentBankMappings: [
    ...
    {
      sourceName: 'Kotak 811',
      paymentMode: 2,
      bankAccountId: KOTAK_BANK_ACCOUNT_ID
    }
  ],
...
writeJson('batch-05-confirm-request-redacted.json', {
  ...confirmPayload,
...
writeJson('batch-05-confirm-result.json', confirmResult)
...
writeJson('batch-05-import-verification.json', verification)
for (const row of verification) log.push({ action: 'verify_imported', ...row })

writeJson('batch-05-action-log.json', log)
writeCsv('batch-05-action-log.csv', log)

const md = [
  '# Sale Import Batch 05 Action Log',
  '',
  'Backup taken before mutation: `garmetix-srp-db-20260804-001425-IST-SaleImportMissingBatch05-v6.9.43.dump`.',
  '',
...
writeFileSync(join(outputDir, 'batch-05-action-log.md'), `${md.join('\n')}\n`, 'utf8')

console.log(JSON.stringify({
  confirmResult,
  verification,
  logPath: join(outputDir, 'batch-05-action-log.md')
}, null, 2))
