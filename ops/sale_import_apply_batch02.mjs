...
const INVOICES = ['AF/2025/1387', 'AF/2025/1390', 'AF/2025/1414', 'AF/2025/1417', 'AF/2025/1442']
const SERVICE_INVOICE = null

const lineActions = new Map([
  ['AF/2025/1387|R24100124|Arvind Mill Shirting TALP52256-1 White Check', {
    action: 'map_existing_barcode',
    overrideBarcode: 'R241001124',
    createProductAndStock: false,
    note: 'Amit approved mapping source R24100124 to barcode R241001124.'
  }],
  ['AF/2025/1387|R240065331|Arvind Mills Shirting White Check TCAME51172-3', {
    action: 'map_existing_barcode',
    overrideBarcode: 'R240065331',
    createProductAndStock: false,
    note: 'Amit approved mapping source R240065331 to barcode R240065331.'
  }],
  ['AF/2025/1387||Arvind TAS  Suiting PW SB12-003784/2 Beige', {
    action: 'create_stock_product',
    overrideBarcode: 'SB12-003784/2',
    createProductAndStock: true,
    note: 'Amit approved creating stock/product for SB12-003784/2 with opening qty same as billed.'
  }],
  ['AF/2025/1390|25024008|RW - Kurta Pajama - 73033/', {
    action: 'map_existing_barcode',
    overrideBarcode: '25024008',
    createProductAndStock: false,
    note: 'Amit approved mapping source 25024008 to barcode 25024008.'
  }],
  ['AF/2025/1390||8907686179179', {
    action: 'create_stock_product',
    overrideBarcode: '8907686179179',
    createProductAndStock: true,
    overrideCategory: 'Garment',
    approvedBrand: 'USPA',
    note: 'Amit approved create stock/product under Garment, brand USPA, size L, opening qty 1.'
  }],
  ['AF/2025/1390||KR59873', {
    action: 'create_stock_product',
    overrideBarcode: 'KR59873',
    createProductAndStock: true,
    overrideCategory: 'Garment',
    approvedBrand: 'SKAM',
    note: 'Amit approved create stock/product, brand SKAM, opening qty same as billed.'
  }],
  ['AF/2025/1390||KR59874', {
    action: 'create_stock_product',
    overrideBarcode: 'KR59874',
    createProductAndStock: true,
    overrideCategory: 'Garment',
    approvedBrand: 'SKAM',
    note: 'Amit approved create stock/product, brand SKAM, opening qty same as billed.'
  }],
  ['AF/2025/1414|26023021|CM - Jodhpuri Suit - 51785/2 42', {
    action: 'map_existing_barcode',
    overrideBarcode: '26023021',
    createProductAndStock: false,
    note: 'Amit approved mapping source 26023021 to barcode 26023021.'
  }],
  ['AF/2025/1414||Ustav Kurta Pajama', {
    action: 'create_stock_product',
    overrideBarcode: 'SKAM0002',
    createProductAndStock: true,
    overrideCategory: 'Garment',
    approvedBrand: 'SKAM',
    note: 'Amit approved create stock/product for Ustav Kurta Pajama, brand SKAM, barcode SKAM0002, opening qty 3.'
  }],
  ['AF/2025/1414|25010022|RW Bundi Kurta-73515-3 SHI/b995/KPENT/Blue-38', {
    action: 'map_existing_barcode',
    overrideBarcode: '25010022',
    createProductAndStock: false,
    note: 'Amit approved mapping source 25010022 to barcode 25010022.'
  }],
  ['AF/2025/1417|38676846241|RNT Fancy Diamond', {
    action: 'mapping_correct',
    overrideBarcode: '38676846241',
    createProductAndStock: false,
    note: 'Amit approved existing mapping as correct.'
  }],
  ['AF/2025/1417|38642617855|RNT Garnet Combo Box', {
    action: 'mapping_correct',
    overrideBarcode: '38642617855',
    createProductAndStock: false,
    note: 'Amit approved existing mapping as correct.'
  }],
  ['AF/2025/1442|26023022|CM - Koti Set - 85619/5,1,4 36', {
    action: 'mapping_correct',
    overrideBarcode: '26023022',
    createProductAndStock: false,
    note: 'Amit approved existing mapping as correct.'
  }],
  ['AF/2025/1442|8907378032782|U.S.POLO ASSN.', {
    action: 'mapping_correct',
    overrideBarcode: '8907378032782',
    createProductAndStock: false,
    note: 'Amit approved existing mapping as correct.'
  }]
])
...
const workbookPath = join(outputDir, 'batch-02-selected-invoices.xlsx')
const log = []
let token = ''
...
async function createServiceInvoice(salesman) {
  ...
  if (order) {
    ...
    order = await api('POST', 'tailoring/orders', {
      ...
      internalRemarks: `VyaparSourceInvoice=${SERVICE_INVOICE}; created by Codex batch-02 approved mapping.`,
  ...
  const updated = await api('PUT', `billing/sales/${converted.id}`, {
    ...
    customerGstin: null,
    salesmanId: salesman.id,
    remarks: `VyaparSaleImport; VyaparSourceInvoice=${SERVICE_INVOICE}; SourceDate=2026-02-20; Service invoice via Tailoring module; CodexBatch=batch-02.`
  })
...
writeJson('batch-02-preview-before-actions.json', preview)
log.push({
  action: 'preview',
...
writeJson('batch-02-confirm-invoices.json', confirmInvoices)
...
writeJson('batch-02-confirm-request-redacted.json', {
  ...confirmPayload,
...
writeJson('batch-02-confirm-result.json', confirmResult)
...
writeJson('batch-02-import-verification.json', verification)
for (const row of verification) log.push({ action: 'verify_imported', ...row })

writeJson('batch-02-action-log.json', log)
writeCsv('batch-02-action-log.csv', log)

const md = [
  '# Sale Import Batch 02 Action Log',
  '',
  'Backup taken before mutation: `garmetix-srp-db-20260803-232843-IST-SaleImportMissingBatch02-v6.9.43.dump`.',
  '',
...
writeFileSync(join(outputDir, 'batch-02-action-log.md'), `${md.join('\n')}\n`, 'utf8')

console.log(JSON.stringify({
  confirmResult,
  verification,
  logPath: join(outputDir, 'batch-02-action-log.md')
}, null, 2))
