...
BS-20 implements only a read-only direct-ledger evidence endpoint at `/api/final-accounts/audit/direct-ledger-integration`. It reads canonical Books `LedgerGroups`/`Ledgers`/`JournalEntries`/`JournalLines`, classifies ledger groups with the BS-17 COA rules, compares against existing Final Accounts statements, and reports mapping rows as exception-only review evidence. Do not switch Final Accounts report sources until remote backup, live evidence and Amit/CA approval are complete.

BS-21 adds the operational restore drill command `npm --prefix frontend/modular run deploy:srp:restore-drill -- --confirm-non-production-restore --stage=BS21RestoreDrillProductionSafety`. It must restore only into `garmetix_restore_drill_*`, verify checksum/history/`pg_restore -l`, run SQL smoke and temporary API `/api/health`, and write `/opt/garmetix/backup/database/RestoreDrillHistory.md`. Do not run production restore or accounting migration until Amit approves that restore evidence.
